...
function Projects() {
    ...
    return ( 
        ...
        <div className={classes.projectsTitleContainer}>
            <h1 className={classes.projectsTitle}>Projects (21-22)</h1>
        </div>

        <ProjectLeft title="Metronome" description={<div><p>A take home task for my position at the music ed-tech startup. Created in React with reliance on setIntervals & setTimeouts, but also learnt about the web audio API which can provide better accuracy for apps that require timing precision.</p><p>First project that required a more structured mathematical process to get the tap tempo timing working, and lining up the animation correctly.</p></div>}
        ...
        <ProjectLeft title="Udemy Homepage Clone" description={<div><p>Purely using HTML and CSS, I cloned the homepage of the popular course selling website Udemy.</p><p>This is fully responsive for all screen sizes and uses the image assets from their site. The icons used are from various resources including Font Awesome. </p><p>I was able to practise using flexbox and grid while improving my general skills in correctly organising layouts and responsiveness using mediaQueries.</p></div>}
